...
@Slf4j
public class DeviceConfigCreationTaskConsumer implements QueueConsumer<Long>, ApplicationContextAware {
    ...
    public TaskExecutionResult execute(@NonNull Task<Long> task) {
        ...
        try {
            deviceConfigurationService.create(deviceId);
            log.info("Configuration file built for device {}", deviceId);
            return TaskExecutionResult.finish();
        } catch (IOException ex) {
            log.warn("Error while creating device configuration for deviceId {}. Error: {}. Task reenqueued for {} seconds.",
            ...
            log.warn("Error while creating device configuration for deviceId {}. Error: {}", deviceId, ex.getMessage());
