...
import otherDeviceTypeResolve from './other-device-type-routing-resolve.service';

describe('OtherDeviceType routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: OtherDeviceTypeService;
  let resultOtherDeviceType: IOtherDeviceType | null | undefined;
  ...
  describe('resolve', () => {
    it('should return IOtherDeviceType returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        otherDeviceTypeResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultOtherDeviceType).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IOtherDeviceType>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
